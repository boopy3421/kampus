...
export default function ItemView() {
  ...
  const product =
    allProducts.find((p) => p.id === Number(id)) ?? allProducts[0];
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.details}>
          ...
          <div className={styles.actions}>
            ...
            {user && !isOwnListing && (
              <Link
                to={`/messages?${messageParams.toString()}`}
          ...
          <div className={styles.meetup}>
            ...
            <div>
              ...
              <p className={styles.meetupSub}>
                CIIT Campus lobby or nearby area
              </p>
            </div>
          </div>
            ...
            <div className={styles.sellerInfo}>
              <p className={styles.sellerName}>
                ...
                {product.sellerUsername && (
                  <span className={styles.sellerUsername}>
                    @{product.sellerUsername}
                  </span>
                )}
              </p>
              <p className={styles.sellerMeta}>
                CIIT Student · {sellerListings.length} active listing
                {sellerListings.length !== 1 ? "s" : ""}
              </p>
            </div>
            <Link
              to={`/profile/${encodeURIComponent(product.sellerId ?? product.seller)}`}
              className={styles.sellerViewBtn}
            >
              View Profile
            </Link>
          </div>
          ...
          <div className={styles.safetyNote}>
            <ShieldCheck size={14} />
            <p>
              Always meet in a safe, public spot on campus. Never send payment
